...
#'
#' The `projr_ignore()` function updates the project’s `.gitignore` and
...
projr_ignore <- function() {
  ...
  .projr_ignore_build_content_dir()
  invisible(TRUE)
}
...
.projr_ignore_devcontainer <- function() {
  if (dir.exists(".devcontainer")) {
    .projr_ignore_dir_rbuild(".devcontainer")
    invisible(TRUE)
  } else {
    invisible(FALSE)
  }
...
.projr_ignore_github <- function() {
  if (dir.exists(".github")) {
    .projr_ignore_dir_rbuild(".github")
    invisible(TRUE)
  } else {
    invisible(FALSE)
  }
...
.projr_ignore_vscode <- function() {
  if (dir.exists(".vscode")) {
    .projr_ignore_dir_rbuild(".vscode")
    invisible(TRUE)
  } else {
    invisible(FALSE)
  }
...
.projr_ignore_file_git <- function(ignore) {
  .projr_ignore_path_add(ignore, .dir_proj_get(".gitignore"))
}
...
.projr_ignore_file_rbuild <- function(ignore) {
  ignore <- gsub("/+$", "", ignore) |>
    trimws() |>
    utils::glob2rx()
  .projr_ignore_path_add(ignore, .dir_proj_get(".Rbuildignore"))
}
...
.projr_ignore_dir_rbuild <- function(ignore) {
  ...
  .projr_ignore_path_add(patterns, .dir_proj_get(".Rbuildignore"))
}
# ===========================================================================
# Ignore specified lines from .gitignore/.Rbuildignore
# ===========================================================================

.projr_ignore_path_add <- function(ignore, path) {
  ...
  file_vec <- .projr_ignore_path_add_get_updated(path, ignore, FALSE)
...
.projr_ignore_path_get_list <- function(path, ignore, override) {
  ...
  if (length(file_vec) == 0) {
    return(.projr_ignore_get_path_list_empty())
  }
...
.projr_ignore_get_path_list_empty <- function() {
...
.projr_ignore_path_get_updated_content <- function(override,
                                                   ignore,
                                                   content) {
  if (override) {
    ignore |> unique()
...
# ===========================================================================
# Ignore specified lines from Git
# ===========================================================================
...
.projr_ignore_add_git <- function(ignore) {
  ...
  gitignore_vec <- .projr_ignore_gitignore_get_updated(ignore, FALSE)
  .projr_ignore_git_write(gitignore_vec, FALSE)
  invisible(TRUE)
}

.projr_ignore_add_gitignore_get_updated <- function(ignore, override) {
  gitignore_list <- .projr_ignore_gitignore_get()

  c(
    gitignore_list[["start"]],
    .projr_ignore_gitignore_get_updated_content(
      override, ignore, gitignore_list[["content"]]
    ),
    gitignore_list[["end"]]
  )
}

.projr_ignore_gitignore_get <- function() {
  # Break the .gitignore file into three parts: the top, the projr-managed
  # section, and the bottom. The projr-managed section is the content between
  # the "Start of projr section" and "End of projr section" comments.
  ...
  # Get the indices of the projr-managed section from the .gitignore content
  ind_vec <- .projr_ignore_gitignore_get_ind(gitignore_vec)

  # Extract the non-projr-managed content from the .gitignore file
  start_end_list <- .projr_ignore_gitignore_get_startend(
    ind_vec[["top"]], ind_vec[["bot"]], gitignore_vec
  )

  # Extract the projr-managed content from the .gitignore file
  content <- .projr_ignore_gitignore_get_content(
    ind_vec[["top"]], ind_vec[["bot"]], gitignore_vec
  )
...
.projr_ignore_gitignore_get_empty <- function() {
  list(start = character(0), content = character(0), end = character(0))
}
...
.projr_ignore_gitignore_get_ind <- function(gitignore_vec) {
  ...
  ind_top <- which(grepl(match_str_top, gitignore_vec))
  ind_bot <- which(grepl(match_str_bottom, gitignore_vec))
  c("top" = ind_top, "bot" = ind_bot)
}

.projr_ignore_gitignore_get_check <- function(match_str_top,
                                                  match_str_bottom,
                                                  gitignore_vec) {
  # Validate that the projr-managed section in .gitignore is well-formed                                                 
...
.projr_ignore_gitignore_get_startend <- function(ind_top,
                                                 ind_bot,
                                                 gitignore_vec) {
  if (length(ind_top) == 0L) {
    start <- c(
    ...
    start <- gitignore_vec[seq_len(ind_top)]
    end <- gitignore_vec[seq(ind_bot, length(gitignore_vec))]
  }
  list(start = start, end = end)
}

.projr_ignore_gitignore_get_content <- function(ind_top,
                                                       ind_bot,
                                                       gitignore_vec) {
  if (length(ind_top) == 0L) {
    character(0)
    ...
    gitignore_vec[seq(ind_top + 1, ind_bot - 1)]
  }
}

.projr_ignore_gitignore_get_updated_content <- function(override,
                                                        ignore,
                                                        content) {
  if (override) {
    ignore |> unique()
...
.projr_ignore_build_content_dir <- function() {
  switch(.projr_engine_get(),
    ...
    "rmd" = NULL, 
    "quarto_document" = .projr_ignore_build_quarto()
  )
}
...
.projr_ignore_build_quarto <- function() {
  fn_vec_qmd <- list.files(pattern = "\\.qmd$")
  path_vec_dir <- NULL
  for (fn in fn_vec_qmd) {
    path_vec_dir <- c(
      path_vec_dir,
      .projr_ignore_build_quarto_ind(fn)
    )
  }
  path_vec_dir <- unique(path_vec_dir)
  .projr_ignore_dir_git(path_vec_dir)
  .projr_ignore_dir_rbuild(path_vec_dir)
  invisible(TRUE)
}

.projr_ignore_build_quarto_ind <- function(fn) {
