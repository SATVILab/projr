.projr_osf_yml_check_title <- function(title) {
  if (is.null(title)) {
...
.projr_osf_yml_check_label <- function(label, type_opt) {
  label_nn <- deparse(substitute(label))
  # check supplied
  if (missing(label)) {
    stop(paste0(
...
.projr_osf_yml_check_public <- function(public) {
  if (!is.logical(public)) {
    stop("public must be logical")
  }
}
.projr_osf_yml_check_body <- function(body) {
...
.projr_osf_yml_check_category <- function(category) {
...
.projr_osf_yml_check_id <- function(id) {
  if (is.null(id)) {
    return(invisible(FALSE))
  }
  if (!length(id) == 1L) {
...
.projr_osf_yml_check_parent_id <- function(parent_id) {
...
.projr_osf_yml_check_parent_title <- function(parent_title) {
...
.projr_osf_yml_check_proj_with_parent <- function(parent_id = NULL,
                                                  parent_title = NULL,
                                                  category) {
  if (is.null(category)) {
    return(invisible(FALSE))
...
.projr_osf_yml_check_overwrite <- function(overwrite) {
  if (!is.logical(overwrite)) {
    stop("overwrite must be logical")
  }
}

.projr_osf_yml_check_path <- function(path) {
  if (is.null(path)) {
    return(invisible(FALSE))
  }
  if (!length(path) == 1L) {
...
.projr_osf_yml_check_path_append_label <- function(path_append_label) {
...
.projr_osf_yml_check_remote_structure <- function(remote_structure,
                                                  nm_opt = NULL) {
...
.projr_osf_yml_check_trans_list <- function(trans_list) {
...
.projr_osf_yml_check_trans_names <- function(trans_list,
                                             nm_opt = NULL,
                                             nm_opt_exc = NULL) {
  if (is.null(nm_opt)) {
...
.projr_osf_yml_check_cue <- function(trans_list,
                                     nm_opt = NULL) {
  direction <- deparse(substitute(trans_list))
...
.projr_osf_yml_check_sync_approach <- function(trans_list,
                                               nm_opt = NULL) {
  direction <- deparse(substitute(trans_list))
...
.projr_osf_yml_check_conflict <- function(trans_list,
                                          nm_opt = NULL) {
  direction <- deparse(substitute(trans_list))
