test_that(".osf_get_path_dir_sub works", {
  # skips
  # setup
  ...
  usethis::with_project(
    path = dir_test,
    code = {
      expect_null(.osf_get_path_dir_sub(
        label = "abc", path_append_label = FALSE, path = NULL
      ))
      expect_identical(
        .osf_get_path_dir_sub(
...
test_that(".osf_complete_dnld_list works", {
  # skips
  # setup
  dir_test <- .test_setup_project(git = TRUE, set_env_var = FALSE)
  ...
  usethis::with_project(
    ...
    code = {
      expect_identical(
        .osf_complete_dnld_list(NULL),
        list("conflict" = "overwrite", "strategy" = "download-all")
      )
      expect_identical(
        .osf_complete_dnld_list(list("conflict" = "abc")),
        list("conflict" = "abc", "strategy" = "download-all")
      )
      expect_identical(
        .osf_complete_dnld_list(
          list("conflict" = "abc", "strategy" = "def")
        ),
        list("conflict" = "abc", "strategy" = "def")
      )
      expect_identical(
        .osf_complete_dnld_list(list("strategy" = "def")),
