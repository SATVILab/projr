...
Environment variables allow you to configure projr projects without
...
as well as for securely storing authentication tokens.
...
adapted for projr’s specific implementation.

## Overview

projr uses environment files to set default values for environment
variables. These files use a simple `KEY=VALUE` format and are read in a
specific order to allow for flexible configuration across different
contexts.
...
1.  **`_environment.local`** - Machine-specific overrides (automatically
    git-ignored)
...
Here’s a typical setup with different configurations for development and
production:
...
scenarios. Both `PROJR_PROFILE` and `QUARTO_PROFILE` are supported.
...
projr recognizes several environment variables that control build
behavior:

### Build Control Variables

**`PROJR_OUTPUT_LEVEL`** - Controls console output verbosity during
builds
...
- **Behaviors**:
  ...
  - `"debug"`: Verbose output including debug messages and remote
    operations details
- Can be overridden by explicit `output_level` parameter in build
  functions
...
- **Behaviors**:
  ...
  - When `FALSE`: Still maintains build history (`builds.md`) but skips
    detailed logs
...
- **Behaviors**:
  ...
  - Special values `"default"` and `"local"` are filtered out (handled
    separately)
...
- **Default**: None
- **Behavior**: Takes precedence over `PROJR_PROFILE` for environment
  file loading
- Only supports comma separation (not semicolon)
...
Environment variables follow a strict precedence order to allow for
...
You can document and validate required environment variables by creating
an `_environment.required` file. This file lists variable names (one per
line) that must be set for builds to proceed properly.
...
Most environment variables are **case-sensitive** (both names and
values):
...
Variable names must follow a specific pattern or they’ll be silently
ignored:
...
If a variable name or value is empty (after trimming whitespace), the
entire line is skipped.

### Profile Names

Avoid using `"default"` or `"local"` as profile names: - These special
values are filtered out automatically by projr - Using them won’t cause
errors, but they’ll be ignored
...
**Remember:** Values are taken as-is after the `=` sign (with
leading/trailing whitespace trimmed). Special characters, spaces, and
symbols are preserved without needing quotes.

### Already-Set Variables

Variables that are already set in your environment are **never
overridden** by files:
...
- [`?projr_build_dev`](https://satvilab.github.io/projr/reference/projr_build_dev.md) -
  Development builds (respects environment variables)
- [`?projr_build`](https://satvilab.github.io/projr/reference/projr_build.md) -
  Production builds (respects environment variables)
...
- [Quarto Environment
  ...
  Quarto’s environment variable documentation
