...
Scripts and hooks customize projr’s build process. Scripts control which
documents to render; hooks run custom R code before or after builds.

Both are configured in `_projr.yml` and have separate settings for
production builds
([`projr_build_patch()`](https://satvilab.github.io/projr/reference/projr_build.md),
etc.) and development builds
...
Scripts are plain character vectors in `_projr.yml`. No sub-keys or
...
When `dev.scripts` is set, it completely replaces `build.scripts` for
dev builds. To include production scripts in dev builds, list them
explicitly under `dev.scripts`.

All specified scripts are validated before the build starts. Missing
files cause an immediate error.
...
Hooks are plain character vectors organized by stage. Three stage keys
are available: `pre` (before the build), `post` (after the build), and
`both` (runs at both stages). File paths are relative to the project
root.
...
Like scripts, all hook files are validated before the build starts. A
missing hook file causes an immediate error. If a hook fails at runtime,
the build stops.
...
- Keep each hook focused on one task. Use descriptive filenames (e.g.,
  `validate-credentials.R` rather than `hook1.R`).
- Wrap external operations in
  [`tryCatch()`](https://rdrr.io/r/base/conditions.html) so error
