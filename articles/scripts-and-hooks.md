...
Build scripts and hooks allow you to customize projr’s build process.
Scripts specify which documents to render, while hooks let you run
custom code before or after builds.

This documentation is modeled after [Quarto’s environment variables
documentation](https://quarto.org/docs/projects/environment.html),
adapted for projr’s build customization features.

## Overview

projr provides two key customization mechanisms:

- **Build scripts** (`build.scripts`, `dev.scripts`) - Control which
  documents/scripts are rendered during builds
- **Build hooks** (`build.hooks`, `dev.hooks`) - Run custom R scripts
  before or after the build process

Both are configured in `_projr.yml` and support separate settings for
production builds (versioned releases) and development builds (iterative
testing).
...
- Explicitly control which documents are built instead of relying on
  auto-detection
- Build only a subset of documents (e.g., skip time-consuming analyses
  during development)
- Override Quarto or Bookdown project configurations

**Build hooks** are useful when you want to:

- Set up data or configuration before builds (e.g., download data, check
  credentials)
- Clean up or process outputs after builds (e.g., compress files, send
  notifications)
- Run validation checks or tests as part of the build process

## Build Scripts Configuration

Build scripts specify exactly which documents or scripts projr should
render during a build.
...
- Use `dev.scripts` if specified
- Fall back to `build.scripts` if no `dev.scripts`
- Fall back to `_quarto.yml` or `_bookdown.yml` if no scripts
  configuration
- Auto-detect documents if no configuration found

**Important**: `dev.scripts` provides *exclusive* control only when
explicitly set. If you set `dev.scripts`, it completely overrides
`build.scripts`. However, if `dev.scripts` is not set at all,
`build.scripts` will be used as a fallback.

### Script Priority Order

For **production builds**:

1.  `file` parameter in build function (e.g.,
    `projr_build_patch("specific.Rmd")`)
...
1.  `file` parameter in build function (e.g.,
    `projr_build_dev("test.Rmd")`)
2.  `dev.scripts` in `_projr.yml`
3.  `build.scripts` in `_projr.yml` (fallback if no `dev.scripts`)
4.  Quarto project (`_quarto.yml`) or Bookdown project (`_bookdown.yml`)
5.  Auto-detection
...
If you have a `_quarto.yml` that lists many documents but only want to
build specific ones:
...
Before any build starts, projr validates that all specified scripts
...
Build hooks let you run custom R scripts at specific points in the build
process.
...
- **`pre`** - Before the build starts (after version bump, before Git
  commit)
- **`post`** - After the build completes (after Git commit, before
  distributing to remotes)
...
- This ensures complete independence between development and production
  workflows
...
- Hooks run in an **isolated child environment** of the global
  environment
- Each hook gets its own fresh environment, preventing interference
  between hooks
...
Like scripts, hooks are validated before the build starts. If a hook
file is missing, the build fails immediately:
...
Build only essential documents during development, but build everything
for releases:
...
Use hooks to configure the environment differently for development and
production:
...
Create hooks that behave differently based on environment variables or
build state:
...
- Use `dev.scripts` to speed up development by building only what you’re
  actively working on
- Rely on auto-detection for simple projects with obvious structure

**Keep paths relative:**

- Use project-relative paths (e.g., `analysis/report.Rmd`) for
  portability
...
If you want `dev.scripts` to *add* to `build.scripts`, you must list all
files:
