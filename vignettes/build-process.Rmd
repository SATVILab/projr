...
```
Production build (patch):
  0.0.1-dev → 0.0.2 → 0.0.3-dev (success)
  0.0.1-dev → 0.0.2 → 0.0.1-dev (failure)

Development build:
  0.0.1-dev → 0.0.1-dev → 0.0.1-dev (no version change)
```
...
- Commit message includes version and "pre-build" designation
...
2. **`dev.scripts`** (dev builds only) - Scripts configured for development
3. **`build.scripts`** (production builds only) - Scripts configured for production
...
# Development scripts (no fallback to build.scripts)
dev:
  scripts:
    - quick-analysis.qmd
```
...
- Commit message includes version and "post-build" designation
...
- Increments to next development version (e.g., "0.0.2" → "0.0.3-dev")
...
```
Build v0.0.2:
  Pre-build:  0.0.1-dev → 0.0.2
  Post-build: 0.0.2 → 0.0.3-dev
```
