...
All builds are automatically logged to `_tmp/projr/log/{dev,output}/history/builds.md`:

```{r eval=FALSE}
# View dev build history
file.edit("_tmp/projr/log/dev/history/builds.md")

# View output build history
file.edit("_tmp/projr/log/output/history/builds.md")
```
...
list.files("_tmp/projr/log/dev/output", 
           recursive = TRUE, pattern = "\\.qmd$")

# Render a log file to HTML
quarto::quarto_render("_tmp/projr/log/dev/output/2025-Nov-11/14-07-00.qmd")
```
...
- **Log directory location**: Logs are in `_tmp/projr/log`, not the regular `_cache` directory.
