...
- **Initialisation**: Run `projr:.init()` to set up the project with metadata, `projr`-specific configuration and a Git repo connected to GitHub (if you so choose).
- **Building the project**: Run `projr:.build_output()` to build the project, version the project and its components, and upload the components to GitHub, OSF and/or a local folder.
...
There are only three functions to remember: .init()`, .build_output()` and .path_get()`.
...
- **Initialisation**: An interactive initialisation function, `projr:.init()`.
...
- **Test builds**: To test build the project, run `projr:.build_dev()`.
...
- **Builds**: To build the project, run `projr:.build_output()`.
...
There are only three functions to remember: .init()`, .build_output()` and .path_get()` (which is described later).
...
One can build the project using .build_output()`.
...
`projr` provides two main functions to help you build your project, .build_output()` and .build_dev()`.
In a nutshell, .build_dev()` is for checking code as you write it, and .build_output()` is for snapshotting and sharing your project.
