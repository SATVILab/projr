...
```{r eval=FALSE}
# Checks in order: github -> osf -> local
projr_restore()
```
...
projr checks remote sources in order:

1. **GitHub Releases** - If configured in `_projr.yml`
2. **OSF** - If configured
3. **Local directories** - If configured

The first available source is used for each artifact type.
