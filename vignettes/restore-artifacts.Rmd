...
**For archive remotes** (most common): Restores the latest available version from the remote, regardless of your current project version.
...
Result: Restores v0.3.0 (latest available on remote)
```

**Note:** To restore a specific older version, check out that Git tag first (`git checkout v0.2.0`), then run `projr_restore()`.
