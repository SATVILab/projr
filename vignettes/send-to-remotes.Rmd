...
**`send_cue = "always"`** (default) - Create new version with every build
...
**`send_cue = "if-change"`** - Only create version when content has changed
...
- Use `"always"` when you want complete version history
- Use `"if-change"` for large datasets that rarely change (saves time and space)
...
- **send_cue**: `"always"` (complete history) or `"if-change"` (efficiency)
